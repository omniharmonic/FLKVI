  ...
  switch (use) {
    ...
    case 'residential-multi':
      ...
      if (old) {
        ...
        return levels <= 4 ? 'brownstone' : 'masonry-tower';
      }
      if (era === '1940-1969' || era === '1970-1999') return levels <= 4 ? (region === 'southwest' ? pick<StyleKit>([['garden-apartments', 2], ['pueblo', 1]], r()) : 'garden-apartments') : 'masonry-tower';
      return levels >= 4 ? 'podium-mixed-use' : 'modern-infill';
    case 'residential-single':
    default: {
      if (area < 45) return 'generic';
      if (density > 0.4 && old && region === 'northeast') return pick<StyleKit>([['brownstone', 3], ['colonial', 1]], r());
...
function materialFor(kit: StyleKit, i: StyleInput, r: () => number): FacadeMaterial {
  ...
  switch (kit) {
    ...
    case 'main-street-block':
      ...
      if (region === 'pacific') return pick<FacadeMaterial>([['stucco', 2.5], ['brick-painted', 1.5], ['lap-siding', 1], ['brick-red', 0.8], ['concrete', 0.5]], r());
...
export function resolveStyle(i: StyleInput): StyleOut {
  ...
  const cols = MATERIAL_COLORS[material];
