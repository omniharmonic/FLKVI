...
function speciesFromTags(g: string, s: string, leaf?: string): string | null {
  const x = g + ' ' + s;
  // palms & desert plants first ('washingtonia' contains 'ash', 'pineapple' contains 'pine')
  if (/washingtonia|fan palm/.test(x)) return 'washingtonia-palm';
  if (/phoenix canariensis|canary/.test(x)) return 'canary-palm';
  if (/phoenix|date palm/.test(x)) return 'date-palm';
  if (/sabal|palmetto|cabbage palm/.test(x)) return 'sabal-palm';
  if (/roystonea|royal palm/.test(x)) return 'royal-palm';
  if (/syagrus|queen palm/.test(x)) return 'queen-palm';
  if (/cocos|coconut/.test(x)) return 'coconut-palm';
  if (/arecaceae|palm/.test(x)) return 'washingtonia-palm';
  if (/carnegiea|saguaro/.test(x)) return 'saguaro';
  if (/parkinsonia|cercidium|palo verde|palo-verde/.test(x)) return 'palo-verde';
  if (/prosopis|mesquite/.test(x)) return 'mesquite';
  if (/olneya|ironwood/.test(x)) return 'ironwood';
  if (/chilopsis|desert willow/.test(x)) return 'desert-willow';
  if (/gleditsia|locust/.test(x)) return 'honey-locust';
  if (/fraxinus|\bash\b/.test(x)) return 'green-ash';
  if (/populus|cottonwood|poplar|aspen/.test(x)) return 'cottonwood';
  if (/picea|spruce/.test(x)) return 'blue-spruce';
  if (/pseudotsuga|douglas/.test(x)) return 'douglas-fir';
  if (/thuja|red cedar|redcedar/.test(x)) return 'western-red-cedar';
  if (/tsuga|hemlock/.test(x)) return 'western-hemlock';
  if (/taxodium|bald cypress/.test(x)) return 'bald-cypress';
  if (/pinus|\bpine\b/.test(x)) return 'ponderosa-pine';
  if (/acer macrophyllum|bigleaf/.test(x)) return 'bigleaf-maple';
  ...
  if (/prunus|cherry|plum/.test(x)) return 'cherry';
  if (/platanus|plane|sycamore/.test(x)) return 'london-plane';
  if (/quercus virginiana|live oak/.test(x)) return 'live-oak';
  if (/quercus agrifolia/.test(x)) return 'coast-live-oak';
  if (/quercus|oak/.test(x)) return 'pin-oak';
  if (/ulmus|elm/.test(x)) return /parvifolia|chinese/.test(x) ? 'chinese-elm' : 'american-elm';
  if (/ginkgo/.test(x)) return 'ginkgo';
  if (/pyrus|pear/.test(x)) return 'callery-pear';
  if (/lagerstroemia|crape|crepe/.test(x)) return 'crape-myrtle';
  if (/magnolia/.test(x)) return 'magnolia';
  if (/eucalyptus/.test(x)) return 'eucalyptus';
  if (/celtis|hackberry/.test(x)) return 'hackberry';
  if (/ficus|banyan/.test(x)) return 'banyan-fig';
  if (/bursera|gumbo/.test(x)) return 'gumbo-limbo';
  if (/coccoloba|sea grape|seagrape/.test(x)) return 'sea-grape';
  if (/bucida|black olive/.test(x)) return 'black-olive';
  if (/lophostemon|brisbane/.test(x)) return 'brisbane-box';
  if (/cupressus|cypress/.test(x)) return 'monterey-cypress';
  if (/juniperus|juniper/.test(x)) return 'ashe-juniper';
  if (/carya|pecan/.test(x)) return 'pecan';
  if (/liriodendron|tulip/.test(x)) return 'tulip-poplar';
